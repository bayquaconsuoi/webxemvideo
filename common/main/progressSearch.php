...
<?php 
  ...
  if(isset($_GET['action'])){
    ...
    $search_type = $_GET['advancedsearch_input'];
      $advancedAction = ($_GET['advancedAction']);
      $category = ($_GET['category']);
      $date_up = ($_GET['date_up']);
      $sort = ($_GET['sort']);

    $search_value = ' "'.$advancedAction.'" Thể loại:"'.$category.'" Ngày đăng: "'.$date_up.'" Sắp xếp theo:"'.$sort.'" ';
  }
...
if (isset ($_GET['name'])) {
  ...
  $type = ($_GET['action']);
  if($type == "videoName"){
    $sql = "SELECT * FROM video WHERE !deleted_at AND (LOWER(tenvideo) LIKE LOWER('%$key%'))";
  } else {
    $sql = "SELECT * FROM video WHERE !deleted_at AND (LOWER(user_name) LIKE LOWER('%$key%'))";
  }
  ...
  if (mysqli_num_rows($rs) > 0) {
    ...
    if (!empty($_GET)) {
      $key = addslashes ($_GET['advancedsearch_input']);
      $advancedAction = ($_GET['advancedAction']);
      $category = ($_GET['category']);
      $date_up = ($_GET['date_up']);
      $sort = ($_GET['sort']);
    ...
    if($sort == "Ngày tải lên") {
      $sort = "created_at";
    } else if($sort == "Lượt xem") {
      $sort = "view_count";
    } else {
      $sort = "like_count";
    }

    if($date_up == "Một giờ qua") {
      $date = "1 HOUR";
    } else if($date_up == "Hôm nay") {
      $date = "24 HOUR";
    } else if($date_up == "Tuần này") {
      $date = "1 WEEK";
    } else if($date_up == "Tháng này") {
      $date = "1 MONTH";
    } else {
      $date = "1 YEAR";
    }

    if($advancedAction == "Tìm theo tên video"){
      $sql = "SELECT * FROM video WHERE !deleted_at AND (LOWER(tenvideo) LIKE LOWER('%$key%')) AND (category = '$category')
      AND (created_at >= now() - INTERVAL $date ) ORDER BY $sort desc ";     
    }
    else {
      $sql = "SELECT * FROM video WHERE !deleted_at AND (LOWER(user_name) LIKE LOWER('%$key%')) AND (category = '$category')
      AND (created_at >= now() - INTERVAL $date ) ORDER BY $sort desc ";       
    }
    $video = executeResult($sql);
    $db = mysqli_connect("localhost", "root", "", "cloneyoutube");
...
<div id="advancedModal" class="advancedModal">
  ...
  <div class="advancedModal-content">
    ...
    <form action="?action=advanced" method="GET">
